...
export class SqlServerCreateStep<T extends ISqlDatabaseConnectionWizardContext> extends AzureWizardExecuteStep<T> {
    ...
    public async execute(context: T & ISubscriptionContext, _progress: Progress<{ message?: string; increment?: number }>): Promise<void> {
    ...
    public shouldExecute(context: T): boolean {
        return !context.sqlServer && !!context.resourceGroup && !!context.newSqlServerName && LocationListStep.hasLocation(<ILocationWizardContext>context);
    }
}
